/**
 * =========================================================================
 * SMART POS & RETAIL CLOUD BACKEND (Google Apps Script)
...
function doPost(e) {
  try {
    ...
    var isPublicAction = (json.action === "SAVE_BILL" || json.action === "SERVICE_REQUEST" || json.action === "CALL_WAITER" || json.action === "DISMISS_SERVICE_REQUEST" || json.action === "RESOLVE_WAITER_CALL" || json.action === "SEND_OTP_EMAIL" || json.action === "CLEAR_TABLE" || json.action === "RESET_TABLE" || json.action === "REGISTER_TRIAL" || json.action === "SUBMIT_INQUIRY");
    ...
    switch (action) {
      ...
      case "REGISTER_TRIAL":
        return handleRegisterTrial(json);

      case "SUBMIT_INQUIRY":
        return handleSubmitInquiry(json);
...
function getOrCreateBillsSheet(ss) {
  var sheet = ss.getSheetByName("Bills") || ss.getSheetByName("Orders") || ss.getSheetByName("Table_Orders") || ss.getSheetByName("Sheet1");
  if (!sheet) {
...
function doGet(e) {
  ...
  if ((sheetId || orgId) && (params.action === "GET_ORDERS" || params.action === "FETCH_ORDERS")) {
    try {
      var ordersMap = {};
      ...
      if (sheetId && sheetId.indexOf("sheet_") !== 0) {
        try {
          ...
          if (sheet) {
            ...
            if (data && data.length > 1) {
              var headers = data[0].map(function(h) { return String(h || "").trim().toLowerCase(); });
              var idIdx = 0, dateIdx = 1, nameIdx = 2, phoneIdx = 3, modeIdx = 4, totalIdx = 7, itemsIdx = 8, statusIdx = 9, tableIdx = 10, txnIdx = 11;
              headers.forEach(function(h, idx) {
                ...
                if (h.indexOf("customer") !== -1 || h.indexOf("name") !== -1) nameIdx = idx;
              ...
              for (var r = 1; r < data.length; r++) {
                ...
                var orderObj = {
                  ...
                  orderId: "BILL_" + rawId,
                  kotNumber: "KOT-" + rawId,
                  customerName: rawName || canonicalTable,
...
function handleSaveBill(data) {
  ...
  if (orgId) {
    try {
      ...
      if (isSettled) {
        ...
        // Keep in memory cache so GET_ORDERS returns it instantly to kitchen & diners
        var cacheKey = "recent_orders_" + orgId.trim();
        var rawCached = props.getProperty(cacheKey);
        var cachedOrders = [];
        if (rawCached) {
        ...
        var orderObj = {
          ...
          customerName: cust.name || b.customer_name || tableName,
        ...
        if (foundIdx !== -1) {
          var existingRank = getStatusRank(cachedOrders[foundIdx].status);
          var incomingRank = getStatusRank(status);
          if (incomingRank >= existingRank) {
            cachedOrders[foundIdx] = orderObj;
          } else {
            orderObj.status = cachedOrders[foundIdx].status;
            cachedOrders[foundIdx] = orderObj;
          }
        } else {
  ...
  try {
    ...
    const rowData = [
      billId,
      timeStr,
      cust.name || b.customer_name || tableName,
...
function handleRegisterTrial(p) {
  var name = p.client_name || p.name || "Customer";
  var shop = p.shop_name || p.shop || (name + " Restaurant");
  var category = p.business_category || p.category || "Restaurant & Cafe";
  var mobile = p.mobile || p.phone || "";
  var email = p.email || "";
  var city = p.address || p.city || "";
  var referral = p.referral_source || p.referral || "";
  var gst = p.gst_no || "";

  var adminEmail = "smartdine.platform@gmail.com";

  // 1. Send Admin Notification Email
  try {
    var adminSubject = "⚡ New 14-Day Free Trial Registration: " + shop + " (" + name + ")";
    var adminHtml = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e2e8f0;border-radius:12px;background:#ffffff;">' +
      '<div style="background:#1e3a8a;color:#ffffff;padding:16px 20px;border-radius:8px;margin-bottom:20px;">' +
        '<h2 style="margin:0;font-size:20px;">🚀 New Free Trial Registration</h2>' +
        '<p style="margin:4px 0 0;font-size:13px;opacity:0.9;">SmartDine Multi-Station Restaurant POS</p>' +
      '</div>' +
      '<table style="width:100%;border-collapse:collapse;font-size:14px;">' +
        '<tr><td style="padding:8px 0;color:#64748b;width:160px;font-weight:bold;">Applicant Name:</td><td style="color:#0f172a;font-weight:bold;">' + name + '</td></tr>' +
        '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">Restaurant Name:</td><td style="color:#0f172a;font-weight:bold;">' + shop + '</td></tr>' +
        '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">Category:</td><td style="color:#0f172a;">' + category + '</td></tr>' +
        '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">Contact Mobile:</td><td style="color:#0f172a;">' + mobile + '</td></tr>' +
        '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">Email Address:</td><td style="color:#0f172a;">' + email + '</td></tr>' +
        '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">City / Area:</td><td style="color:#0f172a;">' + city + '</td></tr>' +
        '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">Discovery Channel:</td><td style="color:#0f172a;">' + referral + '</td></tr>' +
        (gst ? '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">GSTIN:</td><td style="color:#0f172a;">' + gst + '</td></tr>' : '') +
        '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">Trial Tier:</td><td style="color:#16a34a;font-weight:bold;">14-Day Full Feature Trial</td></tr>' +
        '<tr><td style="padding:8px 0;color:#64748b;font-weight:bold;">Registered At:</td><td style="color:#64748b;">' + new Date().toLocaleString() + '</td></tr>' +
      '</table>' +
    '</div>';
    ...
    MailApp.sendEmail({
      ...
      htmlBody: adminHtml,
      name: "SmartDine Web Portal"
    });
  } catch(e) {}

  // 2. Send Client Welcome & Thank-You Email
  if (email) {
    try {
      var clientSubject = "Welcome to SmartDine! Your 14-Day Free Trial is Registered";
      var clientHtml = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:28px;border:1px solid #e2e8f0;border-radius:16px;background:#ffffff;">' +
        '<div style="text-align:center;margin-bottom:24px;">' +
          '<h1 style="color:#0f172a;font-size:24px;margin:0 0 8px;">Welcome to SmartDine POS!</h1>' +
          '<p style="color:#64748b;font-size:14px;margin:0;">The unified system for table QR, kitchen display, waiter stations & counter billing.</p>' +
        '</div>' +
        '<p style="color:#334155;font-size:15px;line-height:1.6;">Hello <strong>' + name + '</strong>,</p>' +
        '<p style="color:#334155;font-size:15px;line-height:1.6;">Thank you for registering <strong>' + shop + '</strong> for the 14-day SmartDine free trial! Your request has been logged in our onboarding queue.</p>' +
        '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:20px;margin:20px 0;">' +
          '<h3 style="margin:0 0 12px;color:#1e293b;font-size:16px;">What happens next?</h3>' +
          '<ol style="margin:0;padding-left:20px;color:#475569;font-size:14px;line-height:1.7;">' +
            '<li>Our onboarding specialist will connect with you on WhatsApp/Phone at <strong>' + mobile + '</strong> to share your store credentials.</li>' +
            '<li>You can run SmartDine on any Android tablet, mobile phone, or computer you already own.</li>' +
            '<li>While waiting, you can test the live guest dining menu demo at <a href="https://smartdine-pos.web.app/r/?org=DEMO&table=1" style="color:#2563eb;font-weight:bold;">smartdine-pos.web.app/r/</a>.</li>' +
          '</ol>' +
        '</div>' +
        '<p style="color:#64748b;font-size:13px;line-height:1.6;">Need urgent setup for tonight\'s service? Reach us directly at <a href="mailto:smartdine.platform@gmail.com" style="color:#2563eb;">smartdine.platform@gmail.com</a>.</p>' +
        '<hr style="border:none;border-top:1px solid #e2e8f0;margin:24px 0;" />' +
        '<div style="text-align:center;color:#94a3b8;font-size:12px;">© 2026 DevMonks Studio · SmartDine Platform Technologies</div>' +
      '</div>';
      ...
      MailApp.sendEmail({
        ...
        htmlBody: clientHtml,
        name: "SmartDine Platform"
      });
    } catch(e) {}
  }

  return responseJson({ success: true, message: "Trial registered successfully." });
}

function handleSubmitInquiry(p) {
  var name = p.name || p.client_name || "Merchant";
  var email = p.email || "";
  var phone = p.phone || p.mobile || "";
  var city = p.city || "";
  var brand = p.brand_name || p.shop_name || "Restaurant Brand";
  var model = p.business_model || p.category || "Restaurant";
  ...
  // 1. Send Admin Notification Email
  try {
    var adminSubject = "💼 Commercial Plan Inquiry: " + brand + " (" + plan + ")";
    var adminHtml = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;border:1px solid #e2e8f0;border-radius:12px;background:#ffffff;">' +
      ...
      '</div>' +
      '<table style="width:100%;border-collapse:collapse;font-size:14px;">' +
        '<tr style="background:#f8fafc;"><td colspan="2" style="padding:8px 12px;font-weight:bold;color:#1e293b;">👤 Basic Contact Details</td></tr>' +
        ...
        '<tr style="background:#f8fafc;"><td colspan="2" style="padding:8px 12px;font-weight:bold;color:#1e293b;">🏢 Business & Operational Details</td></tr>' +
        '<tr><td style="padding:8px 12px;color:#64748b;">Brand / Restaurant:</td><td style="color:#0f172a;font-weight:bold;">' + brand + '</td></tr>' +
        '<tr><td style="padding:8px 12px;color:#64748b;">Format / Model:</td><td style="color:#0f172a;">' + model + '</td></tr>' +
        '<tr><td style="padding:8px 12px;color:#64748b;">Number of Outlets:</td><td style="color:#0f172a;font-weight:bold;">' + outlets + '</td></tr>' +
        '<tr><td style="padding:8px 12px;color:#64748b;">Stations / Hardware:</td><td style="color:#0f172a;">' + stations + '</td></tr>' +
        '<tr><td style="padding:8px 12px;color:#64748b;">Interested Plan:</td><td style="color:#2563eb;font-weight:bold;">' + plan + '</td></tr>' +
        '<tr><td style="padding:8px 12px;color:#64748b;">Specific Notes:</td><td style="color:#334155;">' + reqs + '</td></tr>' +
        '<tr><td style="padding:8px 12px;color:#64748b;">Received At:</td><td style="color:#64748b;">' + new Date().toLocaleString() + '</td></tr>' +
      '</table>' +
    '</div>';
    ...
    MailApp.sendEmail({
      ...
      subject: adminSubject,
      htmlBody: adminHtml,
      name: "SmartDine Sales Desk"
    });
  } catch(e) {}

  // 2. Send Client Thank-You Email
  if (email) {
    try {
      var clientSubject = "Thank you for contacting SmartDine — Plan Inquiry Received";
      var clientHtml = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:28px;border:1px solid #e2e8f0;border-radius:16px;background:#ffffff;">' +
        '<div style="text-align:center;margin-bottom:24px;">' +
          '<h1 style="color:#0f172a;font-size:24px;margin:0 0 8px;">Thank You for Reaching Out</h1>' +
          '<p style="color:#64748b;font-size:14px;margin:0;">SmartDine Enterprise & Restaurant Operations</p>' +
        '</div>' +
        '<p style="color:#334155;font-size:15px;line-height:1.6;">Hello <strong>' + name + '</strong>,</p>' +
        '<p style="color:#334155;font-size:15px;line-height:1.6;">We have received your commercial inquiry regarding <strong>' + plan + '</strong> for <strong>' + brand + '</strong>.</p>' +
        '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:20px;margin:20px 0;">' +
          '<h3 style="margin:0 0 10px;color:#1e293b;font-size:15px;">Summary of Your Inquiry:</h3>' +
          '<ul style="margin:0;padding-left:20px;color:#475569;font-size:13.5px;line-height:1.7;">' +
            '<li><strong>Brand:</strong> ' + brand + ' (' + outlets + ')</li>' +
            '<li><strong>Plan of Interest:</strong> ' + plan + '</li>' +
            '<li><strong>Hardware Stations:</strong> ' + stations + '</li>' +
          '</ul>' +
        '</div>' +
        '<p style="color:#334155;font-size:15px;line-height:1.6;">A dedicated solutions architect from DevMonks will review your requirements and connect with you on <strong>' + phone + '</strong> / <strong>' + email + '</strong> within <strong>2 to 4 business hours</strong> with a customized quote and live demonstration.</p>' +
        '<p style="color:#64748b;font-size:13px;">If you have any immediate questions, feel free to reply directly to this email or write to <a href="mailto:smartdine.platform@gmail.com" style="color:#2563eb;">smartdine.platform@gmail.com</a>.</p>' +
        '<hr style="border:none;border-top:1px solid #e2e8f0;margin:24px 0;" />' +
        '<div style="text-align:center;color:#94a3b8;font-size:12px;">© 2026 DevMonks Studio · SmartDine Platform Technologies</div>' +
      '</div>';
